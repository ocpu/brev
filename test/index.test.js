...
it('registers a function', () => {
  let expected = [mock1]
  expect(bus.listeners(eventName)).toEqual([])
  bus.on(eventName, mock1)
  expect(bus.listeners(eventName)).toEqual(expected)
})
...
it('can\'t register a handler twice', () => {
  expect(bus.listeners(eventName).length).toBe(0)
  bus.on(eventName, mock1)
  expect(bus.listeners(eventName).length).toBe(1)
  bus.on(eventName, mock1)
  expect(bus.listeners(eventName).length).toBe(1)
})
...
it('many executes a function correctly ', () => {
  ...
  expect(bus.listeners(eventName).length).toEqual(1)
  bus.emit(eventName)
  expect(mock1.mock.calls.length).toBe(1)
  expect(bus.listeners(eventName).length).toEqual(1)
  bus.emit(eventName)
  expect(mock1.mock.calls.length).toBe(2)
  expect(bus.listeners(eventName).length).toEqual(0)
  bus.emit(eventName)
})
...
it("cannot register with a empty event string", () => {
  expect(() => bus.many("", 1, () => {})).toThrow(/event must be something/)
  expect(() => bus.on("", 1, () => {})).toThrow(/event must be something/)
  expect(bus.once("", 1, () => {})).rejects.toThrow(/event must be something/)
})
it("cannot register on event that is not a string", () => {
  expect(() => bus.many(1, 1, () => {})).toThrow(/event must be something/)
  expect(() => bus.many(null, 1, () => {})).toThrow(/event must be something/)
  expect(() => bus.many(true, 1, () => {})).toThrow(/event must be something/)
  expect(() => bus.many(() => {}, 1, () => {})).toThrow(/event must be something/)
  expect(() => bus.many(void 0, 1, () => {})).toThrow(/event must be something/)
  expect(() => bus.many(null, 1, () => {})).toThrow(/event must be something/)
  expect(() => bus.on(1, 1, () => {})).toThrow(/event must be something/)
  expect(() => bus.on(null, 1, () => {})).toThrow(/event must be something/)
  expect(() => bus.on(true, 1, () => {})).toThrow(/event must be something/)
  expect(() => bus.on(() => {}, 1, () => {})).toThrow(/event must be something/)
  expect(() => bus.on(void 0, 1, () => {})).toThrow(/event must be something/)
  expect(() => bus.on(null, 1, () => {})).toThrow(/event must be something/)
  expect(bus.once(1, 1, () => {})).rejects.toThrow(/event must be something/)
  expect(bus.once(null, 1, () => {})).rejects.toThrow(/event must be something/)
  expect(bus.once(true, 1, () => {})).rejects.toThrow(/event must be something/)
  expect(bus.once(() => {}, 1, () => {})).rejects.toThrow(/event must be something/)
  expect(bus.once(void 0, 1, () => {})).rejects.toThrow(/event must be something/)
  expect(bus.once(null, 1, () => {})).rejects.toThrow(/event must be something/)
})
...
it('events can be streamed', () => {
  expect(bus.stream(eventName)).toBeDefined()
})
it('streamed events gets called', () => {
  bus.stream(eventName).forEach(mock1)
  expect(mock1).not.toBeCalled()
  bus.emit(eventName, eventValue)
  expect(mock1).toBeCalled()
})
it('events can be streamed and filtered', () => {
  bus.stream(eventName).filter(mock1.mockReturnValue(false).mockReturnValueOnce(true)).forEach(mock2)
  bus.emit(eventName, eventValue)
  bus.emit(eventName, eventValue)
  expect(mock1.mock.calls).toEqual([[eventValue], [eventValue]])
  expect(mock2.mock.calls).toEqual([[eventValue]])
})
it('events can be streamed and mapped to new values', () => {
  bus.stream(eventName).map(mock1.mockReturnValue(eventValue+eventValue).mockReturnValueOnce(eventValue.toLowerCase())).forEach(mock2)
  bus.emit(eventName, eventValue)
  bus.emit(eventName, eventValue)
  expect(mock1.mock.calls).toEqual([[eventValue], [eventValue]])
...
it('observers can unregister themselves', () => {
  var stream = bus.stream(eventName).forEach(mock1)
  bus.emit(eventName, eventValue)
